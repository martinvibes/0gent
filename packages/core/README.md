...
> The agent-native CLI and SDK for 0GENT — real email inboxes, on-chain identity, and pay-per-call AI inference for AI agents on 0G Chain & Celo.
...
**Payments.** Native **0G tokens** on **0G Chain** (chain ID 16661) or **USDC** on **Celo** (chain ID 42220). Pick your chain at setup.

**Custody.** Non-custodial. The seed phrase never leaves your machine. The 0GENT server only ever sees public addresses.

---

**Multi-chain.** Pick your chain at setup. 0G Chain (pay with 0G tokens) or Celo (pay with USDC). Same wallet, same commands, same services.
...
# 2. Fund it with 0G tokens — opens a faucet link + QR (free testnet tokens at faucet.0g.ai)
0gent wallet fund

# 3. Mint your agent identity NFT (0.5 0G)
0gent identity mint

# 4. Claim a real <name>@0gent.xyz inbox (2.0 0G)
0gent email create --name scout

# 5. Send an actual email (0.1 0G)
0gent email send <inbox-id> --to user@example.com --subject "Hi" --body "from the agent"

# 6. Ask an LLM, paid in 0G (0.2 0G)
0gent compute infer "What is 0G Chain in one sentence?"
...
Every paid step is a real on-chain transaction on 0G Chain or Celo. Every resource is owned by your wallet, recorded in `AgentRegistry.sol`, forever.
...
Costs are **0G** (native on 0G Chain) or **USDC** (on Celo), settled at request time via x402. Free commands hit only public read endpoints — no wallet required.
...
| `0gent wallet fund` | free | Print the funding QR + open the faucet link (free testnet 0G at faucet.0g.ai). |
...
|---|---|---|---|
| `0gent identity mint [--name N]` | 0.5 | ✅ | Mints `ZeroGentIdentity` ERC-721 NFT on 0G Chain. One per wallet. Metadata pinned on 0G Storage. |
...
| Command | Cost | Status | Notes |
|---|---|---|---|
| `0gent email create --name N` | 2.0 | ✅ | Provisions `<N>@0gent.xyz`. |
| `0gent email send <id> --to A --subject S --body B` | 0.1 | ✅ | Send a real email. |
| `0gent email read <id> [--limit N] [--compact]` | 0.05 | ✅ | Cards by default; `--compact` for table view. |
| `0gent email threads <id>` | 0.05 | ✅ | List conversation threads. |
...
| `0gent compute infer "<prompt>" [--model M] [--max-tokens N] [--system "..."]` | 0.2 | ✅ | Pay-per-call LLM. Returns OpenAI-shaped `{ response, model, provider, usage }`. |

Sample:

```bash
$ 0gent compute infer "Reply with one short sentence: what is 0G Chain?"
[x402] Paying 0.2 0G on 0G Chain...
...
| `0gent phone provision [<phoneNumber>] [--country US]` | 6.0 | ✅ | Pass a specific E.164 to buy that exact number, or `--country` to grab the first available. CLI pre-validates E.164 before any payment. On Celo: $3.00 USDC. |
| `0gent phone sms <phoneId> --to +1... --body "..."` | 0.1 | ✅ | End-to-end SMS delivery verified. Pre-flight catches To==From, missing fields, and bad E.164 BEFORE x402 charges. |
...
const reply     = await z.computeInfer(                 // 0.2 0G
...
1. Agent       z.computeInfer('what is 0G chain?')
                    │
2. Server      ▶ 402 Payment Required: 0.2 0G to ZeroGentPayment.pay(nonce, "compute-infer")
                    │
3. SDK         ▶ signs tx on 0G Chain or Celo, broadcasts, waits for confirmation
                    │
...
Three contracts on **0G Chain (16661) and Celo (42220)**:
...
- **`Insufficient balance`** on a paid call — For 0G mainnet, fund your wallet with 0G tokens; for testing, free tokens at [faucet.0g.ai](https://faucet.0g.ai). For Celo, fund with USDC.
- **Slow `[x402] Waiting for confirmation`** — Block times on 0G Chain typically settle in ~3s; longer means RPC pressure. The SDK auto-retries.
...
Real. Real on-chain payments. Real emails delivered. Real LLM completions returned. Running on 0G mainnet and Celo.
...
Yes — 0G mainnet (chain 16661) is live. Free testnet 0G is still available at [faucet.0g.ai](https://faucet.0g.ai) if you want to experiment without spending real tokens.

**Does it work on Celo?**
Yes. Run `0gent setup` and select Celo. Payments are in USDC. Same commands, same services.
...
0G Chain doesn't have native USDC, so payments there use the native 0G token. On Celo, USDC is natively available — so Celo payments use USDC instead.
